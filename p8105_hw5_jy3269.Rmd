...
unsolved <- function(citystate){
  homicide %>% 
    ...
    prop.test(x = sum(dataset$disposition == 'Closed without arrest' | dataset$disposition == 'Open/No arrest'), n = length(dataset$disposition)) %>% 
    broom::tidy() %>% 
    select(estimate, conf.low, conf.high)
}
...
```{r}
name = unique(homicide$city_state)

unsolved_result = 
  expand_grid(
    citystate = name
  ) %>% 
  mutate(prop_test_result = map(name, unsolved)) %>% 
